...
import '../../page/P6EDITMASTER/P6EDITMASTERMAIN.dart';
import '../../widget/common/ErrorPopup.dart';
import '../../widget/common/Loading.dart';

//-------------------------------------------------

abstract class P6EDITMASTERGETDATA_Event {}

class P6EDITMASTERGETDATA_GET
    extends P6EDITMASTERGETDATA_Event {}

List<dynamic> dataHead = [];

class P6EDITMASTERGETDATA_Bloc extends Bloc<
    P6EDITMASTERGETDATA_Event,
    List<P6EDITMASTERGETDATAclass>> {
  P6EDITMASTERGETDATA_Bloc() : super([]) {
    on<P6EDITMASTERGETDATA_GET>((event, emit) {
      return _P6EDITMASTERGETDATA_GET([], emit);
    });
  }

  Future<void> _P6EDITMASTERGETDATA_GET(
      List<P6EDITMASTERGETDATAclass> toAdd,
      Emitter<List<P6EDITMASTERGETDATAclass>> emit) async {
    print('IN : P6EDITMASTERGETDATA_GET');
    FreeLoadingTan(P6EDITMASTERMAINcontext);
    List<P6EDITMASTERGETDATAclass> output = [];
    //-------------------------------------------------------------------------------------
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<P6EDITMASTERGETDATAclass> outputdata =
            input.map((dataActual) {
          return P6EDITMASTERGETDATAclass(
        ...
        Navigator.pop(P6EDITMASTERMAINcontext);
        output = outputdata;
        emit(output);
      } else {
        ...
        Navigator.pop(P6EDITMASTERMAINcontext);
        showErrorPopup(
            P6EDITMASTERMAINcontext, response.toString());
        output = [];
        emit(output);
      }
    } catch (e) {
      print(e);
      Navigator.pop(P6EDITMASTERMAINcontext);
      showErrorPopup(P6EDITMASTERMAINcontext, e.toString());
      output = [];
      emit(output);
    }
  }
...
class P6EDITMASTERGETDATAclass {
  P6EDITMASTERGETDATAclass({
    this.ID = '',
    this.MainOrder = '',
    this.OrderNo = '',
    this.Order_Start_DT = '',
